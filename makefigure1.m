...
%% go through all the nuclei
 inuc=12;
    ...
    img_centerline=img_centerline(:,[49:96-hpsiz,hpsiz+1:48]);
    imagesc(img_centerline*0.16,[-0.05,0.2]);colormap jet;hold on;
%     plot([hpsiz hpsiz+64;hpsiz hpsiz+64]-hpsiz,[0 0;250 250],'w');
    xlabel('longitudinal angle')
    ylabel('time (s)')
    title('longitude fluctuation kymograph')
    colorbar;hold on;
...
%     xlabel('longitudinal angle')
%     ylabel('time (s)')
%     title('longitudinal max fluctuation kymograph')
%     colorbar;hold on;
...
%     FigureFormat(gcf)
    ...
    %% std map
    imagesc(std_img(hpsiz+1:end-hpsiz,[49:96-hpsiz,hpsiz+1:48])*0.16,[0.02 0.15]);colormap jet;axis image;colorbar;hold on;
%                    plot([hpsiz hpsiz+64;hpsiz hpsiz+64],[1 1;19 19],'r'); hold on;
    ...
    FigureFormat(gcf);
        %% single frame
    imagesc(squeeze(time_img(hpsiz+1:end-hpsiz,[49:96-hpsiz,hpsiz+1:48],50))*0.16,[-0.06 0.15]);colormap jet;axis image;colorbar;hold on;
%                    plot([hpsiz hpsiz+64;hpsiz hpsiz+64],[1 1;19 19],'r'); hold on;
%     if ~isempty(tracks_ext)
%         for itj=1:tracks_ext(end,end)
%             trtmp=tracks_ext(tracks_ext(:,end)==itj,:);
...
%         end
%     end
    title('fluctuation height map at max peak frame (50th frame)');
    xlabel('longitudinal angle')
    ylabel('latitudinal angle')
    set(gca,'XTick',(0:16:64));
    set(gca, 'xticklabel', '0 | p/2 | p | 3p/2 | 2p', 'fontname', 'symbol');
