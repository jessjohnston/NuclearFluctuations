...
%use 0.2 drift corrected data
path='C:\nuclei\post analysis result_0.2';
load([path,'\','alltracks.mat'])
...
%     display(['processing ',name]);
    if strcmp(ext,'.mat') && strcmp(name,'heh2_MBC_07')
...
%                 end
            ...
            nucind1{savingindex}=[name,'_',num2str(inuc)];
            trj1{savingindex}=tracks_ext; 
...
%             img_centerline=[img_centerline(:,end-hpsiz+1:end),img_centerline,img_centerline(:,1:hpsiz)];
            imagesc(img_centerline*0.16,[-0.05,0.2]);colormap jet;
            xlabel('angle')
            ylabel('frames')
...
%             max_longi=[max_longi(:,end-hpsiz+1:end),max_longi,max_longi(:,1:hpsiz)];
            ...
            imagesc(std_img(hpsiz:end-hpsiz,:)*0.16,[0.02 0.15]);colormap jet;axis image;colorbar;hold on;
            hold on;
            if ~isempty(tracks_ext)
                 for itj=1:tracks_ext(end,end)
                    trtmp=tracks_ext(tracks_ext(:,end)==itj,:);
...
trj=[trj,trj1];
nucind=[nucind,nucind1];
save([path,'\','alltracks.mat'],'trj','nucind')
