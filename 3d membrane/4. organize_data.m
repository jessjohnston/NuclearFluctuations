...
%     typeind=find(contains(gnmovienames(:,1),name)); % Patch to work with
%         % folder names that do not necessarily match strain name, e.g.,
%         % gnmovienames could be '2682x10-2_gcn5_Cut11-GFP_01_R3D' (folder)
%         % and name could be 'gcn5' (strain). Merely want to search for
%         % similar data found in folders with slightly different incremental
%         % names. - JJ 11/15/2017
    nucsave=1;
    clear nucleitmp;
    %     nucleitmp=[];
...
%         movieid=movieids(imovie);
        ...
        load(fullfile(rootpath,'data',gnmovienames{imovie,1},...
            gnmovienames{imovie,3}));
        numnuc=nm.num_nuc;
        ...
        for inuc=1:numnuc
            ...
            nuctmp.cellvolume=(nuctmp.celllength-nuctmp.cellwidth)*...
                pi/4*nuctmp.cellwidth^2+pi/6*(nuctmp.cellwidth).^3;
...
%                 centerdrift(iframe,:)=(rot_mat*cnt')';
                centerdriftraw(iframe,:)=[nuc.origin_new(1:2),...
                    nuc.origin_new(3)*zxr];
                %nuclei radius
                ...
                elseif cnt(3)<=1
                    ...
                    nucsize(iframe)=sqrt((lowerarea*lowerweight+...
                        upperarea*upperweight)/pi);
                end
            end
...
%                         subfluc.latitude=(subtrj(:,2)-hpsiz-10)/64*2*pi;
    ...
    if exist('nucleitmp')
        if ~isempty(nucleitmp)
            ...
            warning(['no nuclei available for ',name]);
        end
    end
end
