classdef psfmov < dvmov
    ...
    methods
        function obj=psfmov()
            obj=obj@dvmov('.\Archive\@psfmov\PSF_02_R3D.dv');
        end
        ...
        function [sigdiff,minsig,ppsf]=psfxzdiff(psfobj)
            ...
            psfzxr=psfobj.vox/psfobj.pix;
            %             bpsfimg=bpass3(psfimg,filterparam.lnoise,filterparam.lobject,psfzxr);
            bpsfimg=bpass3(psfimg,.5,2,psfzxr);
            ...
            for i=1:size(psfpk,1)
                ...
                if cx>10 && cy>10 && cz>10 && cx<nx-10 &&cy<ny-10 && cz<nz-10
                    %                 wpsfimg=psfimg(cy-10:cy+10,cx-10:cx+10,cz-10:cz+10);
                    ...
                    gauss3=@(p) exp(-((psfX-p(1)).^2+(psfY-p(2)).^2)/p(4)^2-(psfZ-p(3)).^2/p(5)^2)*p(6)-wpsfimg;
